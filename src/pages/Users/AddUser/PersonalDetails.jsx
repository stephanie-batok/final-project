...
export default function PersonalDetails(props) {
    ...
    const handleNext = () => {
        if(getValues('id')!=="" && getValues('first_name')!=="" && getValues('last_name')!==""&& getValues('email')!==""&& getValues('password')!=="" && getValues('user_type')!==""){
            setError("");
        ...
        <React.Fragment>
            <form>
                <Grid container spacing={4}>
                    ...
                    <Grid item xs={12} sm={6}>
                        <TextField select label="*בחר סוג משתמש" onChange={(e) => register({name:"user_type", value: e.target.value})} fullWidth>
